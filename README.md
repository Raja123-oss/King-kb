...
<title>King KB — Game Dadu (Poin)</title>
<style>
...
<div class="container">
  <div class="card center">
    <div class="huge">🎲 KING KB</div>
    <div class="footer">Multi-user • Save data • WebView-ready</div>
...
<div id="result" class="center big" style="margin-top:8px">—</div>
...
function login(){const u=loginUser.value.trim(), p=loginPass.value.trim(); if(!users[u]||users[u].pass!==p) return alert('Login gagal'); current=u; auth.classList.add('hidden'); game.classList.remove('hidden'); uName.textContent=u; uSaldo.textContent=users[u].saldo; renderPlayHist()}
...
function pick(v){picked=v; btnK.classList.toggle('active',v==='K'); btnB.classList.toggle('active',v==='B')}
...
function renderPlayHist(){const box=playHist; box.innerHTML=''; (users[current].plays||[]).slice(0,10).forEach(p=>{const it=document.createElement('div'); it.className='item'; it.innerHTML=`<span>${new Date(p.t).toLocaleTimeString()} • ${p.pick}</span><b>${p.res} (${p.delta})</b>`; box.appendChild(it)})}
...
function renderReqs(){reqList.innerHTML=''; reqs.forEach((r,i)=>{const it=document.createElement('div'); it.className='item'; it.innerHTML=`<span>@${r.u} • ${r.amt}</span><button class='btn gray' onclick='approve(${i})'>SELESAI</button>`; reqList.appendChild(it)})}
function approve(i){const r=reqs.splice(i,1)[0]; if(users[r.u]) users[r.u].saldo+=r.amt; save(); renderReqs(); alert('Top up poin selesai')}

function topupAdmin(){const u=topUser.value.trim(), v=Number(topVal.value||0); if(!users[u]||v<=0) return alert('Data salah'); users[u].saldo+=v; save(); alert('OK')}
